...
import SkletonLoader from "../skletonLoader/SkletonLoader";

const Movie = () => {
  const { id } = useParams();
  const dispatch = useDispatch();
  const { isLoading, movieDetail } = useSelector((state) => {
  ...
  return <div>{isLoading ? <SkletonLoader /> : movieDetailPage}</div>;
};
